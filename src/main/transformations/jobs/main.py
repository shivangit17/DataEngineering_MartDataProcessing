...
from pyspark.sql.types import IntegerType, StructField, StructType, StringType, DateType, FloatType
...
# aws_access_key = "BoDD3/AeLUlf6/nzi0HdA5X/qL6piMZkOEZSw7+YowE="
...
if csv_files:
    ...
    if data:
        ...
        logger.info("No data found in present file")
else:
...
logging.info(f"file path available on s3 under {bucket_name} and folder name is  {file_path}")
try:
...
# get a list of all file in a local directory
...
logger.info("******************** Listing of Files ********************************** ")
logger.info("List of csv files that need to processed %s", csv_files)
...
logger.info(
    f"************ Updating the product_staging_table that we have started the process ************"
)
...
# final_df_to_process = database_client.create_dataframe(
#     spark,
#     table_name="empty_df_create_table"
# )
...
for data in correct_files:
    ...
    extra_columns = list(
        set(data_schema) - set(config.mandatory_columns)
    )
    ...
    if extra_columns:
        ...
        logger.info(
            f"processed {data} and added 'additional_column'"
        )
...
logger.info(
    f"**************** customer data written to local disk at "
    f"{config.customer_data_mart_local_file} **************"
)

#Move data on s3 bucket for customer_data_mart

logger.info(
    f"**************** Data Movement from local to s3 for customer data mart **************"
)
...
logger.info(
    f"*************** sales team data written to local disk at "
    f"{config.sales_team_data_mart_local_file}"
)

# Move data on s3 bucket for sales_data_mart
s3_directory = config.s3_sales_datamart_directory

message_ = s3_uploader.upload_to_s3(
    s3_directory,
    config.bucket_name,
    config.sales_team_data_mart_local_file
)
...
if correct_files:
    ...
    logger.info(
        f"Updated statement created for staging table --- {update_statements}"
    )

    logger.info(
        "****************Connecting with My SQL server ************************"
    )
    ...
    logger.info(
        "**************** My SQL server connected successfully ****************"
    )
...
input("Press double enter to terminate ")
